...
export class ScrumboardService {
  ...
  createBoard(board: Board): Observable<Board> {
    return this.boards$.pipe(
      take(1),
      switchMap((boards) =>
        this._httpClient.put<Board>('api/apps/scrumboard/board', { board }).pipe(
          map((newBoard) => {
  ...
  deleteBoard(id: string): Observable<boolean> {
    return this.boards$.pipe(
      take(1),
      switchMap((boards) =>
        this._httpClient.delete('api/apps/scrumboard/board', { params: { id } }).pipe(
  ...
  createLabel(label: Label): Observable<Label> {
    return this.board$.pipe(
      take(1),
      switchMap((board) =>
        this._httpClient.post<Label>('api/apps/scrumboard/board/label', { label }).pipe(
          map((newLabel) => {
